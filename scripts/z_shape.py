...
def generate_data(M, D, shape, Func_name, r, data_dir):
    gwas_file = os.path.join(data_dir, f'gwas-{shape}-{Func_name}-r{r}.csv')
    anno_file = os.path.join(data_dir, f'anno-{shape}-{Func_name}-r{r}.csv')
    if not os.path.exists(gwas_file) or not os.path.exists(anno_file):
        Func = eval(Func_name)
        zs, pvals, A, Z = Generate_data_disc(M, D, Func, shape)
        # save data
...
def simu_shape(M, D, Func_name, J, v, gam, lam, r, data_dir, out_dir):
    
    # PALM-Tree
    command = f'python ./PALM/prioritize.py \
    --out {out_dir}/{shape}-{Func_name}-J{J}-v{v}-gam{gam}-lam{lam}-r{r} \
    --sumstats {data_dir}/gwas-{shape}-{Func_name}-r{r}.csv \
    --annotation {data_dir}/anno-{shape}-{Func_name}-r{r}.csv \
...
if __name__ == '__main__':
    ...
    data_dir = os.path.join(data_root, f'shape_M{M}_D{D}_K{K}')
    ...
    for shape in shapes:
        for Func_name in Func_names:
            for r in range(1, nrep+1):
                generate_data(M, D, shape, Func_name, r, data_dir)
            ...
            for J in Js:
                ...
                try:
                    with Pool(nrep) as pool:
                        pool.starmap(simu_shape, inp_args)
                        pool.close()
                        pool.join()
