...
def simu_initial_random(true_alpha, Func_name, J, v, gam, lam, r, data_dir, out_dir, alpha, f0):
    
    # PALM-Tree
    command = f'python ./PALM/prioritize.py \
    --out {out_dir}/alpha{true_alpha}-{Func_name}-J{J}-v{v}-gam{gam}-lam{lam}-r{r}-random \
        ...
        for Func_name in Func_names:
            ...
            for J in Js:
                for r in range(1, nrep+1):
                    alpha = np.random.uniform(*alpha_interval)
                    f0 = scipy.special.logit(np.random.uniform(*pi0_interval))
                    inp_args.append((true_alpha, Func_name, J, v, gam, lam, r, data_dir, out_dir, alpha, f0))

            try:
                with Pool(nrep) as pool:
                    pool.starmap(simu_initial_random, inp_args)
                    pool.close()
                    pool.join()
